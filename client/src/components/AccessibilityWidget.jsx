...
// Floating accessibility toolbar — appears bottom left of every page
// Lets users adjust how the site looks and behaves to suit their needs
// All settings are saved to localStorage so they persist after refresh

import { useState, useEffect } from "react";

// Default settings — all features off, normal font size
const DEFAULTS = {
  fontSize:        0,      // font size offset: -1 = smaller, 0 = normal, 1 = larger, 2 = largest
  highContrast:    false,  // increases contrast across the whole page
  grayscale:       false,  // removes all colour from the page
  highlightLinks:  false,  // adds amber outline around all links
  pauseAnimations: false,  // stops all CSS animations and transitions
  dyslexicFont:    false,  // switches font to Lexend (easier to read)
};

// Loads saved preferences from localStorage when the page loads
// If nothing is saved yet, returns the defaults above
function loadPrefs() {
  try {
    return { ...DEFAULTS, ...JSON.parse(localStorage.getItem("a11y_prefs") || "{}") };
  } catch {
    return DEFAULTS;
  }
}
...
export default function AccessibilityWidget() {
  ...
  // Runs every time prefs changes
  // Applies the current settings directly to the <html> element
  useEffect(() => {
    const html = document.documentElement;

    // Font size — offsets the base font size in pixels
    // -1 = -2px, 0 = normal, 1 = +2px, 2 = +4px
    const sizes = [-2, 0, 2, 4];
    html.style.fontSize = prefs.fontSize !== 0
      ? `calc(1rem + ${sizes[prefs.fontSize + 1]}px)`
      : "";

    // High contrast and grayscale can be combined using CSS filter
    ...
    // Pause animations — injects a style tag that overrides animation durations to near zero
    ...
    // Highlight links — injects a style tag that adds an amber outline to all <a> tags
    ...
    // Dyslexia font — loads Lexend from Google Fonts and applies it to all text
    ...
    // Save current prefs to localStorage so settings survive page refresh
    localStorage.setItem("a11y_prefs", JSON.stringify(prefs));
  }, [prefs]);

  // Updates a single preference key with a new value
  function update(key, val) {
    setPrefs((p) => ({ ...p, [key]: val }));
  }

  // Resets all preferences back to defaults and removes all injected styles
  function reset() {
    setPrefs(DEFAULTS);
  ...
  // Reusable toggle button used for each on/off feature
  // Shows blue background when active, grey border when inactive
  const Toggle = ({ label, value, onChange, icon }) => (
    <button
      ...
      {/* Visual toggle track and sliding thumb */}
  ...
  return (
    <div className="fixed bottom-5 left-5 z-50 flex flex-col items-start gap-3">
      ...
      {/* Accessibility panel — only shown when open is true */}
      {open && (
        <div className="w-72 bg-white rounded-2xl shadow-2xl border border-slate-200 overflow-hidden">
          ...
          <div className="p-4 space-y-4">
            ...
            {/* Font size — decrease and increase buttons with a visual level bar */}
            <div>
              ...
              <div className="flex items-center gap-2">
                ...
                {/* Visual bar — each segment lights up blue at the current level */}
            ...
            {/* Display toggles — each controls a different visual feature */}
            <div className="space-y-2">
              <p className="text-xs font-bold text-slate-500 uppercase tracking-wide mb-2">Display</p>

              {/* Doubles CSS contrast on the whole page */}
              ...
              {/* Removes all colour from the page */}
              ...
              {/* Adds amber outline to all clickable links on the page */}
              ...
              {/* Sets all animation and transition durations to near zero */}
              ...
              {/* Loads Lexend font from Google Fonts and applies it to all text */}
            ...
            {/* Resets everything back to defaults and clears localStorage */}
      ...
      {/* Toggle button — always visible at bottom left, opens and closes the panel */}
      <button
        onClick={() => setOpen((o) => !o)}
