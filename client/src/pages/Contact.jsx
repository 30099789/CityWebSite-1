...
// Assessment requirement: Contact form wired to real API (POST /api/contact)
// Assessment requirement: client-side and server-side form validation
// Assessment requirement: field-level error messages shown inline
// Assessment requirement: success/error state feedback to the user
// Contact details loaded from static data — matches settings.xml configuration

import { useState } from "react";
import BASE_URL from "../services/api";

// ── Email validation regex ─────────────────────────────────────────────────────
// Assessment requirement: validate email format before submitting
const EMAIL_REGEX = /^[^\s@]+@[^\s@]+\.[^\s@]+$/;

// ── Client-side validation ─────────────────────────────────────────────────────
// Assessment requirement: validate all required fields before API call
...
// ── Contact details ────────────────────────────────────────────────────────────
// Assessment requirement: contact information for CityLink Initiatives
// Matches the contact details in settings.xml and the portal footer
const CONTACT_ITEMS = [
  {
...
export default function Contact() {
  ...
  const [sent, setSent]             = useState(false);    // true after successful submission
  const [apiError, setApiError]     = useState("");       // server-side error message

  // Clear field-level error when user starts correcting a field
  ...
  // ── Submit handler ─────────────────────────────────────────────────────────
  // Assessment requirement: form submission to REST API with error handling
  // POST /api/contact — sends name, email, subject, message to server
  // Server validates fields again before saving (double validation)
  ...
  // Dynamic input class — shows red border on validation error
    ...
    <main className="min-h-screen bg-slate-50">
      ...
      <div className="max-w-5xl mx-auto px-4 py-8">
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
          ...
          {/* Contact form — assessment requirement: functional form with validation */}
            ...
            <div className="bg-white rounded-2xl border border-slate-200 shadow-sm p-6 sm:p-8">
              ...
              {sent ? (
                ...
                <>
                  ...
                  {/* API error — shown when server returns an error response */}
                  {apiError && (
                  ...
                  {/* noValidate disables browser validation — we use our own field-level validation */}
                  <form onSubmit={handleSubmit} className="space-y-4" noValidate>
                    ...
                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                      ...
                      {/* Email field — required, validated against EMAIL_REGEX */}
                      <div>
                        <label className="block text-sm font-semibold text-slate-800 mb-1.5">
